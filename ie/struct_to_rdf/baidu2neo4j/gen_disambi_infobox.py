...
#        print "type line: ", type(lines[0].encode("utf-8"))
        lines = [Clean.clean_word(line.decode('utf-8'), clean_level='title') for line in lines]
        return lines
...
def main():
    with open("./410_baidu/410_disambi_infobox.csv") as inf:
        lines = inf.readlines()
        f = open("./410_baidu/410_disambi_infobox_out.csv", "w")
        list_attr = []
            ...
            try:
                ...
                for attr in info_dict.keys():
                    ...
                    for v in value_split:
                        v = Clean.clean_word(v.decode('utf-8')).strip(u"等").strip(u"收起")
                        title_list.append(v)
                        f.write("\"" + disambi.encode('utf-8') + "\",\"" + clean_attr + "\",\"" + v.encode('utf-8') + "\"" + "\r\n")
        ...
        with open("./sort_counts.txt", "w") as ouf:
            for i in sort_counts:
                ouf.write(str(i) + "\n")
        with open("./all_attr.txt", "w") as ouf:
            for word_counts in sort_counts:
                if  word_counts[1] >= 10:
                    ouf.write(str(word_counts[0]) + "\n")
        with open("./410_baidu/410_title_new.csv", "w") as ouf:
            for i in title_list:
                ouf.write("\"" + i + "\"\r\n")
        with open("./410_baidu/all_attr.txt", "w") as ouf:
            for i in list_attr:
                ouf.write(i + "\n")
