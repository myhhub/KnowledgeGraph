...
def transfer_json(in_file_path, out_file_path):
    with open(in_file_path) as inf:
        ...
        for line_num in tqdm(range(int(word_num))):
            ...
            word_dict["word"] = words[0].decode('utf-8')
            word_dict["vec"] = eval("[" + ",".join(words[1:]) + "]")
            word_embed_list.append(word_dict)
        json.dump(word_embed_list, ouf)

if __name__ == "__main__":
    seg_file(infile="6w_clean_disambi_text.csv", outfile="seg_6w_disambi_text.txt")
    os.system("./thirdpart/word2vec/word2vec  -train seg_6w_disambi_text.txt -output word_vec.txt -size 50 -window 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -cbow 0 -iter 3 -min-count 1 -hs 1")
